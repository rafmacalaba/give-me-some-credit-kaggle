...
def weight_optimizer_generator(y_train, preds):
    def weight_optimizer(coeffs):
        return roc_auc_score(y_train, combine_models_with_weights(preds, coeffs))
...
def get_oof_agg(model_test_preds, model_type, res_name, agg='median'):
    test_oof = model_test_preds[
        model_test_preds.columns[
            model_test_preds.columns.str.contains('{}_{}'.format(model_type, res_name))
        ]
    ]
...
def get_optimized_weighted_preds_for(preds_dict, num_models=5):
    train = preds_dict['train']
    test = preds_dict['test']
    y_train = preds_dict['y_train']
    ...
    print(
        roc_auc_score(y_train, optimized_weights_preds),
        roc_auc_score(y_train, combine_models_with_weights(train, np.mean(coeffs, axis=0)))
    )
