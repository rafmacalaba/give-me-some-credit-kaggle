...
def cv_train_model(
    ...
    train_val_auc = roc_auc_score(y_train, y_oof_preds)
    ...
    opt_val_auc = roc_auc_score(y_opt_val, opt_val_preds.mean(axis=1))

    if not X_opt_val.empty:
        logging.info('{} auc score: {}'.format(model_id, train_val_auc))

    return y_oof_preds, classifiers, opt_val_auc
